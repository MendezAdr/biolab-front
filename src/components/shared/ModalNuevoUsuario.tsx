...
interface ModalNuevoUsuarioProps {
  ...
  onGuardar: (nuevo: Usuario) => void; // El puente: envía el usuario creado al panel principal
}

export function ModalNuevoUsuario({ isOpen, onClose, onGuardar }: ModalNuevoUsuarioProps) {
  // Estados para capturar lo que el usuario escribe en los inputs
  const [nombre, setNombre] = useState('');
  const [cedula, setCedula] = useState('');
  const [correo, setCorreo] = useState('');
  const [rol, setRol] = useState<'Administrador' | 'Bioanalista' | 'Recepcionista'>('Recepcionista');
  const [activo, setActivo] = useState<boolean>(true);
  const [ultimoAcceso, setFecha] = useState('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const usuarioCreado: Usuario = {
      ...
      nombre: nombre,
      cedula : cedula, // Campo adicional que no se solicita en el modal
      correo: correo, // Campo adicional que no se solicita en el modal
      rol: rol, // Rol por defecto
      activo: activo, // Activo por defecto
      ultimoAcceso: ultimoAcceso , // Usamos la fecha como último acceso
    };

    onGuardar(usuarioCreado); // Enviamos los datos al componente padre
    ...
    setFecha(new Date().toLocaleString()); // Actualizamos la fecha al momento de creación
      ...
      */}
      <div className="relative bg-white rounded-xl shadow-xl border border-slate-200 max-w-md w-full p-6 transition-all transform scale-100">
        ...
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-slate-800">Generar Nuevo Usuario</h3>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              value={rol}
              onChange={(e) => setRol(e.target.value as 'Administrador' | 'Bioanalista' | 'Recepcionista')}
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-emerald-500 bg-slate-50"
            >
